...
│   └── classes.txt              casualty · vehicle · person (순서가 곧 클래스 번호)
...
- **클래스는 셋이다** (`tools/autolabel.py` 의 `CLASSES` — 나머지 도구는 전부 여기서 끌어다 쓴다):

  | 키 | 번호 | 이름 | 색 | 무엇 |
  |---|---|---|---|---|
  | `1` | 0 | `casualty` | 빨강 | **인명피해가 있는 요구조자** — 쓰러진·깔린·움직이지 않는 사람 |
  | `2` | 1 | `vehicle` | 파랑 | 차량 |
  | `3` | 2 | `person` | 노랑 | 일반 사람 — 서 있거나 걷는, 구조대원 포함 |

  요구조자를 일반 사람과 나눈 이유는 이 대회가 묻는 것이 "사람이 몇인가"가 아니라
  **"구조가 필요한 사람이 어디 있는가"** 이기 때문이다. 한 클래스로 두면 모델이 찾아낸 것
  중에 무엇이 급한지를 가릴 수 없다.

  모형은 **그것이 대신하는 것**으로 라벨한다 — 쓰러진 모형은 `casualty`, 서 있는 모형은
  `person`. "모형사람" 같은 클래스는 만들지 않는다.
...
- **`casualty` 는 자동으로 절대 안 깔린다.** COCO 는 쓰러진 사람과 서 있는 사람을 구분하지 않아 자동 라벨의 사람은 전부 `person`(노랑)이다. 요구조자인 상자를 골라 `1` 로 바꾸는 것이 손라벨의 핵심이다.
...
| `Delete` | 선택한 상자 지우기 |
| `1` / `2` / `3` | 클래스 바꾸기 (casualty 빨강 / vehicle 파랑 / person 노랑). 선택 중이면 그 상자의 클래스도 같이 바뀐다 |
