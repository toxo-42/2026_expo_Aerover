...
그래서 자동으로 깔린 사람 상자는 **전부 `person`(일반)이다.** 그중 어느 것이
`casualty`(요구조자)인지는 사람만 안다 — 라벨 도구에서 그 상자를 골라 `1` 을 눌러
빨강으로 바꾸는 것이 이제 손라벨의 핵심 작업이다.
...
# **순서가 곧 클래스 번호다.** 한 번 정하면 바꾸지 않는다 — 바꾸면 이미 그려둔 .txt 의
# 번호가 전부 다른 것을 가리킨다.
#
# `casualty` 는 **인명피해가 있는 요구조자**(쓰러진·깔린·움직이지 않는 사람)이고,
# `person` 은 멀쩡히 서 있거나 걷는 일반 사람(구조대원 포함)이다. 둘을 나누는 이유는
# 이 대회가 묻는 것이 "사람이 몇인가"가 아니라 **"구조가 필요한 사람이 어디 있는가"**
# 이기 때문이다. 한 클래스로 두면 모델이 찾아낸 것 중 무엇이 급한지를 가릴 수 없다.
#
# **사전학습 모델은 이 구분을 못 한다.** COCO 의 person 은 전부 `person`(일반)으로 깔린다
# (`src/core/detect.py` 의 COCO_KEEP). `casualty` 는 사람이 손으로 지정한다.
CLASSES = ["casualty", "vehicle", "person"]
